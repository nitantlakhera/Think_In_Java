package nitant.JavaSE.Java7.Multithreading.DeadLock;

public class lockInSameOrder {
	...
	public static class thread_two extends Thread{
		 public void run() {
			 synchronized (lock1) {
				System.out.println("Thread two has lock1");
				try {
					Thread.sleep(100);
			  } catch (InterruptedException e) {
			  }
				System.out.println("Thread two waiting for lock two");
				synchronized (lock2) {
				System.out.println("Thread two has lock two");	
				}
